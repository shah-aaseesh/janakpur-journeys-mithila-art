...
const Testimonials: React.FC = () => {
  ...
  const handleBookTourClick = () => {
    window.open("https://api.whatsapp.com/send/?phone=9779766115966&text&type=phone_number&app_absent=0", "_blank");
  };
      ...
      <div className="container mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
          {testimonials.map((testimonial) => (
            <div
              key={testimonial.id}
              className="bg-mithila-cream p-4 sm:p-6 rounded-lg shadow-md border-l-4 border-mithila-red animate-fade-in"
            >
              <div className="flex items-center mb-3 sm:mb-4">
                <div className="flex">{renderStars(testimonial.rating)}</div>
              </div>
              <p className="text-gray-700 mb-4 sm:mb-6 italic text-sm sm:text-base leading-relaxed break-words">"{testimonial.comment}"</p>
              <div className="flex items-center">
                <div className="w-10 h-10 sm:w-12 sm:h-12 bg-mithila-indigo rounded-full flex items-center justify-center text-white font-bold overflow-hidden flex-shrink-0">
                  <img 
                ...
                <div className="ml-3 min-w-0 flex-1">
                  <p className="font-bold text-mithila-indigo text-sm sm:text-base break-words">{testimonial.name}</p>
                  <p className="text-xs sm:text-sm text-gray-600 break-words">from {testimonial.origin}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="mt-8 sm:mt-12 text-center">
          <button 
            className="inline-block px-4 sm:px-6 py-3 bg-mithila-green text-white rounded-md font-medium hover:bg-opacity-90 transition-all transform hover:-translate-y-1 text-sm sm:text-base touch-manipulation"
