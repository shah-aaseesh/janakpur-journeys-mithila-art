      ...
      <div className="container mx-auto px-4 sm:px-6 py-6 sm:py-8 relative z-10">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-8 lg:gap-2">
          <div className="text-center lg:text-left w-full lg:w-1/2 animate-fade-in">
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold font-hind mb-4 text-mithila-indigo leading-tight break-words">
              <span className="text-mithila-red">Welcome to</span> 
              <br />Travel Janakpur
            </h1>
            <h2 className="text-lg sm:text-xl md:text-2xl font-medium mb-4 text-mithila-black break-words">
              Your Trusted Travel Partner in Janakpur
            </h2>
            <p className="text-sm sm:text-base md:text-lg text-gray-700 mb-6 max-w-lg mx-auto lg:mx-0 leading-relaxed break-words">
              Discover authentic Mithila culture and heritage with Nepal's leading travel agency. Professional guides, curated experiences, and unforgettable journeys await.
            </p>
            <div className="flex flex-col sm:flex-row justify-center lg:justify-start gap-3 sm:gap-4">
              <a
                href="#contact"
                className="px-4 sm:px-6 py-3 bg-mithila-red text-white rounded-md font-medium hover:bg-opacity-90 transition-all transform hover:-translate-y-1 text-center"
                onClick={(e) => {
                  e.preventDefault();
              ...
              <a
                href="#services"
                className="px-4 sm:px-6 py-3 border-2 border-mithila-indigo text-mithila-indigo rounded-md font-medium hover:bg-mithila-indigo hover:text-white transition-all transform hover:-translate-y-1 text-center"
                onClick={(e) => {
                  e.preventDefault();
          ...
          <div className="w-full lg:w-1/2 flex justify-center lg:justify-end animate-fade-in mt-6 lg:mt-0">
            <div className="relative">
              {/* Agency showcase image - Janakpur temple */}
              <div className="w-56 h-56 sm:w-64 sm:h-64 md:w-72 md:h-72 lg:w-80 lg:h-80 bg-mithila-cream rounded-lg overflow-hidden border-4 border-mithila-yellow shadow-lg">
                <img 
                  src="/hero-image.jpg" 
                  alt="Travel Janakpur - Cultural Heritage" 
                  className="w-full h-full object-cover"
                  loading="eager"
                />
              </div>
              {/* Decorative elements */}
              <div className="absolute -bottom-3 -right-3 sm:-bottom-4 sm:-right-4 w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 bg-mithila-yellow rounded-full -z-10"></div>
              <div className="absolute -top-3 -left-3 sm:-top-4 sm:-left-4 w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 bg-mithila-green rounded-full -z-10"></div>
              
              {/* Agency badge */}
              <div className="absolute top-3 left-3 sm:top-4 sm:left-4 bg-mithila-red text-white px-2 py-1 sm:px-3 sm:py-1 rounded-full text-xs sm:text-sm font-medium">
                Since 2021
              </div>
            </div>
          </div>
